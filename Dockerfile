...
RUN mkdir -p /home/ffbo/.ffbo/config && \
    ...
    echo "LD_LIBRARY_PATH=/usr/local/lib:/usr/local/cuda/lib64:/usr/lib:/usr/lib/x86_64-linux-gnu:$LD_LIBRARY_PATH" | tee -a ~/.bashrc && \
    echo "OMPI_MCA_opal_cuda_support=true" | tee -a ~/.bashrc
